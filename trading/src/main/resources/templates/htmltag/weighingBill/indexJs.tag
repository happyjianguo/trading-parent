                ...
                $(res.rows).each(function (i, row) {
                    ...
                    $(visibleColumns).each(function (index, item) {
                        if (item.field.indexOf('.') > 0) {
                            ...
                            obj[item.field.replace('\.', '_')] = row[item.field];
                            if (!obj[item.field.replace('\.', '_')]) {
                                let strs = item.field.split('\.');
                                obj[item.field.replace('\.', '_')] = row[strs[0]][strs[1]];
                            }
                        } else {
                            if (i <= 0) {
                                printObj.columns.push({
...
// callbackObj.printDirect(JSON.stringify(data.data),"SettlementPieceDocument");
    ...
    $(function () {
        ...
        _grid.bootstrapTable('refreshOptions', {
            ...
            columns: JSON.parse(localStorage.getItem('weightingBillGridVisibleColumns')).data
        });
    });
